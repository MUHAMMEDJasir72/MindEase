...
class Notification(models.Model):
    user = models.ForeignKey(
        UserDetails, on_delete=models.CASCADE, related_name="notifications"
    )
...
class TherapistTransaction(models.Model):
    ...
    wallet = models.ForeignKey(
        Wallet, on_delete=models.CASCADE, related_name="therapist_transactions"
    )
    transaction_type = models.CharField(
        max_length=10, choices=TRANSACTION_TYPES)
    amount = models.IntegerField()
...
class TherapistNotification(models.Model):
    user = models.ForeignKey(
        UserDetails, on_delete=models.CASCADE, related_name="therapist_notifications"
    )
    message = models.TextField()
    read = models.BooleanField(default=False)
    time = models.DateTimeField(auto_now_add=True)
...
class AdminNotification(models.Model):
    user = models.ForeignKey(
        UserDetails, on_delete=models.CASCADE, related_name="admin_notifications"
    )
    message = models.TextField()
    read = models.BooleanField(default=False)
    time = models.DateTimeField(auto_now_add=True)
